...
from mytale.sessions.backends.db import SessionStore as DbStore
...
class PHPSerialize(object):
    ...
    def serialize_key(self, data):
        ...
        elif type(data) is types.StringType or type(data) == unicode:
            if self.is_int(data):
                return "i:%s;" % int(data)
            else:
...
class SessionStore(DbStore):
    ...
    def decode(self, session_data):
        s = PHPUnserialize()
        return s.session_decode(session_data)
